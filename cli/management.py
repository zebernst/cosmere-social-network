...
def refresh(args: Namespace):
    ...
    if args.dataset.lower() == "list":
        print(
            "    characters                   refreshes the character data from coppermind.net"
        )
    else:
        if args.dataset.lower() in ("coppermind", "characters", "wiki"):
            ...
            if paths.coppermind_cache_path.is_file():
                ...
                delta = [
                    rev
                    for rev in old_data + new_data
                    if rev not in old_data or rev not in new_data
                ]
                ...
                if delta:
                    ...
                    fp = (paths.log_dir / "deltas" / f"{time_str}.log").open(
                        "w", encoding="utf-8"
                    )
                    ...
                    for _, grp in groupby(sorted(delta, key=page_id), key=page_id):
                        ...
                        if len(grp) == 1:
                            ...
                            if old["title"] != new["title"]:
                                print(
                                    f"[[{old['title']}]] renamed to [[{new['title']}]]."
                                )
                                fp.write(
                                    f"[[{old['title']}]] renamed to [[{new['title']}]].\n"
                                )
