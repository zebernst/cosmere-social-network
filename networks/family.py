        ...
        for relations in (char.relatives[k] for k in FIELDS if k in char.relatives):
            for relation in relations:
                ...
                if relation in names:
                    ...
                    logger.debug(
                        f"Identified exact target of edge ({char.name}, {relation})."
                    )
                ...
                # check if characters share a surname
                    ...
                    logger.debug(
                        f"Identified likely target of edge ({char.name}, {relation}) "
                ...
                # last resort: loop through all characters and match substrings
                    ...
                    for name, other in monikers.items():
                        if relation in name:
                            ...
                            found = True
                            logger.debug(
                                f"Could not confirm target of edge ({char.name}, {relation}), "
                                f"assuming {target.name}."
                            )
                            break
                    if not found:
                        logger.info(
                            f"Could not identify target of edge ({char.name}, {relation})."
                        )
