...
class Character:
    ...
    def __init__(self, query_result: dict):
        ...
        if self._keep:
            logger.debug(
                f"Created {self.name} ({self.world if self.world else 'Unknown'})."
            )
        else:
            logger.debug(
                f"Ignored {self.name} ({self.world if self.world else 'Unknown'})."
            )
    ...
    def _parse_infobox(self, result: dict) -> dict:
        ...
        def parse_books(wikicode: mwp.wikicode.Wikicode):
            return [
                b
                for b in (
                    books.get(b.text.strip_code() if b.text else b.title.strip_code())
        ...
        if content:
            ...
            if wiki_template:
                infobox = next(
                    (t for t in wiki_template if t.name.strip().lower() == "character"),
                    None,
                )
                ...
                for entry in infobox.params:
                    key, val = (
                        re.sub(r"[^a-z\-]", "", str(entry.name).lower()),
                        entry.value,
                    )
